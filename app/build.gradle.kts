...
// Empty on purpose: this fork must not offer the original author's APK as an update.
// Set this to your own release manifest URL if you publish updates.
...
android {
    ...
    signingConfigs {
        ...
        // Fixed, repo-committed debug keystore. Without this, AGP falls back to a
        // per-machine auto-generated debug keystore; on ephemeral CI runners (GitHub
        // Actions) that means a NEW signing certificate on every build, so installing
        // a freshly built APK over a previously installed one fails
        // (INSTALL_FAILED_UPDATE_INCOMPATIBLE) and forces an uninstall first. Using
        // the same keystore on every build keeps the signature stable so new builds
        // install as an in-place update instead.
        getByName("debug") {
