...
class RuntimeExecutionService : Service() {
    ...
    private var notificationTitle: String = "Rlaude Harness is working"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action ?: ACTION_START) {
            ...
            ACTION_COMPLETE -> finishTask(
                ...
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "Rlaude Harness finished working in $projectName.",
                failed = false,
            )
            ACTION_FAILED -> finishTask(
                title = "Task needs attention",
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "Rlaude Harness could not finish the task.",
                failed = true,
            )
            ACTION_CANCELLED -> {
                taskRunning = false
        ...
        fun ensureNotificationChannels(context: android.content.Context) {
            ...
            manager.createNotificationChannel(
                NotificationChannel(RUNNING_CHANNEL_ID, "Running coding tasks", NotificationManager.IMPORTANCE_LOW).apply {
                    description = "Shows progress while Rlaude Harness is working in the background"
                },
            )
            manager.createNotificationChannel(
