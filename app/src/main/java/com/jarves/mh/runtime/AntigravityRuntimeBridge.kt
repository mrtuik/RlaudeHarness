...
class AntigravityRuntimeBridge(
    ...
    override suspend fun startSession(
        ...
        runCatching {
            ...
            val request = JSONObject()
                .put("event", "user")
                .put("message", JSONObject().put("content", antigravityWorkspacePrompt(projectSlug, prompt, lessons.buildLessonsPromptSection())))
                .toString() + "\n"
            process.outputStream.write(request.toByteArray())
            process.outputStream.flush()
            process.outputStream.close()
...
internal fun antigravityWorkspacePrompt(projectSlug: String, prompt: String, pastMistakesSection: String): String = """
    ...
    WORK NARRATION AND REAL-TIME UPDATES:
    Do not silently run a long chain of tool calls (view_file, run_command, etc.) and only speak once at the very end with a final summary.
    Instead, periodically pause between tool calls and write a short plain-language sentence (outside any tool call) explaining what you just did and why, before moving to the next step (for example: "Checking the current file structure to find where the attachment images are stored.").
    Do this at natural checkpoints (roughly every 2 to 4 tool calls, or whenever moving from one sub-task to the next), not after every single tool call so it does not get noisy.
    Keep these updates short (one or two plain sentences, no code, no jargon dump) so the user can follow along in real time while steps are running.
    Still give a final plain-language summary at the end as you already do, but that should no longer be the only explanation in the conversation.
