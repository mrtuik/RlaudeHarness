...
fun TerminalScreen(
    ...
    Scaffold(
        ...
        topBar = {
            if (compactHeader) {
                Column(Modifier.background(if (isDark) Color(0xFF0A0A0A) else Color.White)) {
                    Row(
                        ...
                        Box(
                            modifier = Modifier
                                .size(34.dp)
                                .background(if (isDark) Color(0xFF222222) else Color.White, RoundedCornerShape(10.dp))
                                .border(1.dp, if (isDark) Color(0xFF333333) else Color(0xFFE2E8F0), RoundedCornerShape(10.dp)),
                            contentAlignment = Alignment.Center,
                        ) {
                            Icon(Icons.Default.Terminal, contentDescription = null, tint = if (isDark) Color.White else Color.Black, modifier = Modifier.size(18.dp))
                        }
                ...
                TopAppBar(
                    ...
                    colors = TopAppBarDefaults.topAppBarColors(containerColor = if (isDark) Color(0xFF0A0A0A) else Color.White),
                )
            }
        },
    ) { padding ->
        Column(
                ...
                Row(
                    ...
                    quickCommands.forEach { cmd ->
                        AssistChip(
                            ...
                            border = androidx.compose.foundation.BorderStroke(1.dp, if (isDark) Color(0xFF333333) else Color(0xFFE2E8F0)),
                            label = {
                                Text(
                                    cmd,
            ...
            val terminalBg = if (isDark) Color(0xFF0D0D0D) else Color.White
            val promptGreen = if (isDark) MaterialTheme.colorScheme.primary else Color(0xFF535353)
            val commandTextColor = if (isDark) Color(0xFFF5F5F5) else MaterialTheme.colorScheme.onSurface
            val outputTextColor = if (isDark) Color(0xFFD0D0D0) else MaterialTheme.colorScheme.onSurface
            val emptyStateColor = if (isDark) Color(0xFF757575) else MaterialTheme.colorScheme.onSurfaceVariant
            ...
            Surface(
                modifier = Modifier
                    ...
                    .border(1.dp, if (isDark) MaterialTheme.colorScheme.outlineVariant.copy(alpha = 0.4f) else Color(0xFFE2E8F0), RoundedCornerShape(12.dp)),
                color = terminalBg,
...
private fun TerminalCommandPrompt(promptPath: String, command: String, isDark: Boolean = true) {
    val promptGreen = if (isDark) MaterialTheme.colorScheme.primary else Color(0xFF535353)
    val commandColor = if (isDark) Color(0xFFF5F5F5) else MaterialTheme.colorScheme.onSurface
