...
class AppPreferences(private val context: Context) {
    ...
    var themeMode: String
        get() = preferences.getString("theme_mode", "light") ?: "light"
    ...
    var cachedLessonsJson: String
        get() = preferences.getString("cached_lessons_json", "[]") ?: "[]"
        set(value) { preferences.edit().putString("cached_lessons_json", value).apply() }

    var lastLessonsFetchMillis: Long
        get() = preferences.getLong("last_lessons_fetch_millis", 0L)
        set(value) { preferences.edit().putLong("last_lessons_fetch_millis", value).apply() }

    var githubLessonsRepo: String
        get() = preferences.getString("github_lessons_repo", "mrtuik/RlaudeHarnessAgent") ?: "mrtuik/RlaudeHarnessAgent"
        set(value) { preferences.edit().putString("github_lessons_repo", value).apply() }
    ...
    fun saveMessages(projectId: String, chatId: String, messages: List<ChatMessage>) {
        ...
        messages.forEach { m ->
            arr.put(JSONObject().apply {
                ...
                put("changedFiles", JSONArray().apply {
                    m.changedFiles.forEach { put(it) }
                })
            })
        }
        val projectDir = File(chatsDir, projectId).also { it.mkdirs() }
    ...
    fun deleteProjectChats(projectId: String) {
        ...
        val draftPrefix = "draft_${projectId}_"
        val keys = preferences.all.keys.filter { it.startsWith(draftPrefix) }
        if (keys.isNotEmpty()) {
            preferences.edit().apply { keys.forEach(::remove) }.apply()
        }
    }
    ...
    private fun loadLegacyMessages(file: File): List<ChatMessage> {
        ...
        return runCatching {
            ...
            (0 until arr.length()).map { i ->
                val obj = arr.getJSONObject(i)
                val parsedWorkItems = obj.optJSONArray("workItems")?.let { workItems ->
                ...
                val hasWrites = parsedWorkItems.any { item ->
                ...
                val rawChangedFiles = obj.optJSONArray("changedFiles")?.let { changedArray ->
                    (0 until changedArray.length()).mapNotNull { idx ->
                        runCatching { changedArray.getString(idx) }.getOrNull()
                    }
                }.orEmpty()
                ChatMessage(
                    id = obj.getString("id"),
                    ...
                    changedFiles = if (hasWrites) rawChangedFiles else emptyList(),
                )
            }
        }.getOrDefault(emptyList())
    }

    fun saveDraft(projectId: String, chatId: String, text: String, attachments: List<ChatAttachment>) {
        val key = "draft_${projectId}_$chatId"
        ...
        val obj = JSONObject().apply {
            ...
            put("attachments", JSONArray().apply {
                attachments.forEach { att ->
                    put(JSONObject().apply {
                        put("id", att.id)
                        put("displayName", att.displayName)
                        put("relativePath", att.relativePath)
        ...
        preferences.edit().putString(key, obj.toString()).apply()
    }

    fun loadDraft(projectId: String, chatId: String): Pair<String, List<ChatAttachment>> {
        val key = "draft_${projectId}_$chatId"
        val raw = preferences.getString(key, null) ?: return Pair("", emptyList())
        return runCatching {
            val obj = JSONObject(raw)
            val text = obj.optString("text", "")
            val attachments = obj.optJSONArray("attachments")?.let { arr ->
            ...
            Pair(text, attachments)
        }.getOrDefault(Pair("", emptyList()))
    }

    fun clearDraft(projectId: String, chatId: String) {
        preferences.edit().remove("draft_${projectId}_$chatId").apply()
    }
